...
import { ArrowLeft, Share, Heart, Star, MapPin, Phone, Plus, ShoppingBag } from 'lucide-react';
...
export default function ShopDetail() {
  ...
  if (!shop) return <div className="p-8 text-center">매장을 찾을 수 없습니다.</div>;
    ...
    <div className="relative mx-auto min-h-screen max-w-[430px] bg-[#F7FAFC] pb-32 font-sans text-slate-900">
      ...
      <div className="relative -mt-6 rounded-t-[2rem] bg-[#F7FAFC] px-4 pt-6 z-20">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-[#1a212d]">{shop.name}</h1>
            <div className="mt-2 flex items-center gap-2">
              <span className={cn(
                "flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider",
                shop.is_open ? "bg-green-100 text-green-700" : "bg-slate-200 text-slate-500"
              )}>
                <span className={cn("size-1.5 rounded-full", shop.is_open ? "bg-green-500" : "bg-slate-400")}></span>
        ...
        <div className="mt-5 flex gap-3">
          ...
          <button 
            ...
            onClick={() => shop.phone ? window.location.href = `tel:${shop.phone}` : toast({title: "전화번호가 없습니다"})}
          >
      ...
      <div className="min-h-[500px]">
        {categories.map((cat) => (
          <div key={cat.id} id={cat.id} className="scroll-mt-32 px-4 pt-8 first:pt-6">
            <h2 className="mb-4 text-xl font-bold text-[#1a212d]">{cat.name}</h2>
            <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-1 flex-col justify-between py-1">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {item.is_popular && <span className="text-[10px] bg-red-100 text-red-600 px-1.5 py-0.5 rounded font-bold">HOT</span>}
                      </div>
                    ...
                    <div className="flex items-center justify-between pt-2">
                      <span className="font-bold text-[#1a212d]">{item.price.toLocaleString()}원</span>
                      {!item.is_sold_out ? (
                        <button 
                        ...
                        <span className="text-xs font-bold text-red-500">품절</span>
                      )}
                    </div>
                  </div>
              ...
              {cat.menu_items.length === 0 && <div className="text-center py-4 text-slate-400 text-sm">준비 중입니다.</div>}
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="fixed bottom-0 left-1/2 z-50 w-full max-w-[430px] -translate-x-1/2 p-4 pb-8">
          <div className="flex items-center justify-between rounded-full bg-[#1a212d] p-2 pl-6 text-white shadow-2xl shadow-[#1a212d]/30">
            <div className="flex flex-col">
              <span className="text-xs font-medium text-slate-400">{totalItems}개 메뉴</span>
              <span className="text-lg font-bold leading-none">{totalPrice().toLocaleString()}원</span>
            </div>
            <button 
